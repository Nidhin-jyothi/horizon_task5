...
def integrate_with_second_program(named_points):
    ...
    if shortest_path:
        ...
        for i in shortest_path:
            arunList.append(i)
            print(i)
            print(named_points[i])
            arunTuple.append(list(named_points[i]))
        for i in range(1,len(arunList)):
                print('*******',arunList[i-1],'to',arunList[i])
                print(arunTuple[i-1])
                y1=arunTuple[i-1][1]
                x1=arunTuple[i-1][0]
                y2=arunTuple[i][1]
                ...
                print("ANGLE:",angle)
        print("TOTAL DISTANCE IS:",countdis)
